...
TEST(VendorPacketTest, getterTest) {
  auto test_packet = TestVendorPacket::Make(get_capabilities_request);

  ASSERT_EQ(test_packet->GetCompanyId(), BLUETOOTH_COMPANY_ID);
  ASSERT_EQ(test_packet->GetCommandPdu(), CommandPdu::GET_CAPABILITIES);
